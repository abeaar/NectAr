...
final class SimulationSceneController {
    ...
    func select(_ selection: SimulationPlaybackSelection) {
        ...
        let plan = RoundTripPlan(
            ...
            router: router,
            deviceB: deviceB,
            deviceAInRange: RouterRangeChecker.isInRange(device: deviceA, router: router, range: attributes.range),
            deviceBInRange: RouterRangeChecker.isInRange(device: deviceB, router: router, range: attributes.range)
        )
    ...
    private func runFullLoop(plan: RoundTripPlan, arView: ARView) async {
        ...
        if plan.deviceAInRange && plan.deviceBInRange {
            origin = plan.deviceA
            let aToRouter = WallObstructionChecker.isObstructed(from: plan.deviceA, to: plan.router, planes: planes)
            let routerToB = WallObstructionChecker.isObstructed(from: plan.router, to: plan.deviceB, planes: planes)
            ...
            origin = plan.deviceA
            let obstructed = WallObstructionChecker.isObstructed(from: plan.deviceA, to: plan.router, planes: planes)
            waypoints = [plan.router, plan.deviceA]
            waypointsObstructed = [obstructed, obstructed]
        } else {
            origin = plan.deviceB
            let obstructed = WallObstructionChecker.isObstructed(from: plan.router, to: plan.deviceB, planes: planes)
    ...
    private func runMailLoop(origin: simd_float4x4, waypoints: [simd_float4x4], waypointsObstructed: [Bool], arView: ARView) async {
        do {
            let mail = try await MailEntityLoader.load()
